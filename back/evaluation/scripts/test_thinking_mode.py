...
async def _call_litellm(
    ...
    if not parsed:
        return None, None
    answers = {
        k: bool(v) for k, v in parsed.get("answers", {}).items() if isinstance(k, str)
    }
...
async def main(criterion: str | None) -> None:
    ...
    for case in cases:
        ...
        checklist_prompt = _build_checklist_prompt(
            crit, case["prompt"], case["response"]
        )

        print("    no_think … ", end="", flush=True)
        ...
        rows.append(
            (case_id, crit, agr_nt, agr_th, ans_nt or {}, ans_th or {}, rsn_nt, rsn_th)
        )
    ...
    for case_id, crit, nt, th, *_ in rows:
        ...
        print(
            col.format(
                case_id[:36], crit, f"{nt:.2f}", f"{th:.2f}", f"{sym} {delta:+.2f}"
            )
        )
    ...
    print("=" * 60)
    print(
        f"{'criterion':<22}  {'no_think':>10}  {'thinking':>10}  {'delta':>8}  {'n':>4}"
    )
    print("-" * 60)
    ...
    for crit in sorted(by_crit):
        ...
        sym = "▲" if delta > 0 else ("▼" if delta < 0 else "=")
        print(
            f"{crit:<22}  {mean_nt:>10.3f}  {mean_th:>10.3f}  {sym}{delta:>+7.3f}  {len(pairs):>4}"
        )
    ...
    print("-" * 60)
    print(
        f"{'GLOBAL':<22}  {g_nt:>10.3f}  {g_th:>10.3f}  {'▲' if g_d > 0 else '▼' if g_d < 0 else '='}{g_d:>+7.3f}  {len(rows):>4}"
    )
    print()
    ...
    if g_d > 0.05:
        print(
            "→ Consider moving ollama/qwen3:1.7b from _NO_THINK_MODELS to _THINK_MODELS."
        )
    elif g_d < -0.05:
        print(
            "→ Keep ollama/qwen3:1.7b in _NO_THINK_MODELS (thinking degrades accuracy)."
        )
    else:

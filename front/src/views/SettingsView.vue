...
const ROUTING_STRATEGIES = [
  {
    value: 'best_score',
    label: 'Best score',
    description: 'Model with highest aggregate score for active profile and use case',
  },
  {
    value: 'progression',
    label: 'Best progression',
    description: 'Model whose score improves most across recent evaluations',
  },
  {
    value: 'stability',
    label: 'Stability',
    description: 'Minimises score variance — preferred for GDPR-sensitive or critical workloads',
  },
  {
    value: 'strict',
    label: 'Strict compliance',
    description: 'Excludes any model below threshold on any active criterion',
  },
]
  ...
  store.config.profiles.push({
    ...
    criteria_config: Object.fromEntries(
      store.config.criteria
        ...
        .map((c) => [
          c.id,
          { enabled: true, weight: c.weight, calibration_notes: '', min_score: null },
        ]),
    ),
  })
...
function setCalNotes(profileId: string, criterionId: string, value: string) {
  ...
  if (!profile.criteria_config[criterionId]) {
    profile.criteria_config[criterionId] = {
      enabled: true,
      weight: 1.0,
      calibration_notes: value,
      min_score: null,
    }
  } else {
              ...
              <button class="accordion-header" @click="toggleProfile(profile.id)">
                ...
                <div class="accordion-right">
                  <span class="accordion-count"
                    >{{ Object.keys(profile.criteria_config).length }} criteria</span
                  >
              ...
              <div v-if="expandedProfile === profile.id" class="accordion-body">
                          ...
                          <input
                            ...
                            @input="
                              (e) => {
                                ...
                                profile.criteria_config[criterion.id] = {
                                  ...(profile.criteria_config[criterion.id] ?? {
                                    enabled: true,
                ...
                <div class="panel-section">
                  ...
                  <div class="panel-section-desc">
                    Assign a judge model + persona to each domain. Arena calibration data informs
                    which judge is most reliable per criterion.
                  </div>
                  <div class="panel-judges">
                    <div
                      v-for="(judge, ji) in panelFor(profile.id)"
                      :key="ji"
                      class="panel-judge-row"
                    >
                      <div class="panel-judge-top">
                        ...
                        <button
                          ...
                          @click="removeJudgeFromPanel(profile.id, ji)"
                        >
                          ✕
                        </button>
                      </div>
                      ...
                      <div class="panel-criteria-grid">
                        <label
                          v-for="c in store.config.criteria"
                          :key="c.id"
                  ...
                  <div class="panel-actions">
                    <button class="add-dashed-btn" @click="addJudgeToPanel(profile.id)">
                      + Add judge to panel
                    </button>
                    <button
                      class="btn-auto-assign"
                      @click="autoAssignPanel(profile.id)"
                      title="Pre-fill panel using best judge per criterion from ground truth validity data"
                    >
                      Auto-assign from ground truth
                    </button>
                  </div>
                  <div v-if="generatorConflicts(profile.id).length" class="panel-warning">
                    ⚠
                    {{
                      generatorConflicts(profile.id)
                        .map((m) => m.replace('ollama/', ''))
                        .join(', ')
                    }}
                    {{ generatorConflicts(profile.id).length === 1 ? 'is' : 'are' }} also the
                    preferred generator for a use case using this profile — a model should not judge
                    its own outputs.
                  </div>
                </div>
              </div>
              ...
              <div v-if="expandedUseCase === uc.id" class="accordion-body">
                <div class="uc-config">
                  ...
                  <div class="uc-config-row">
                    ...
                    <input
                      type="number"
                      v-model.number="uc.min_score_threshold"
      ...
      <template v-if="activeTab === 'judge'">
        ...
        <section class="settings-section">
          <div class="section-header">
            ...
            <div class="section-desc">
              Models used as judges in Arena mode. All selected judges evaluate the same prompt
              simultaneously — inter-judge variance (σ) is computed across them. Empty = all
              available models.
            </div>
          </div>
          <div class="arena-panel-list">
            <label v-for="m in availableModels" :key="m" class="arena-judge-row">
              ...
              <span class="arena-judge-active" v-if="store.config.arena_judge_models.includes(m)"
                >in panel</span
              >
            </label>
          </div>
      ...
      <template v-if="activeTab === 'routing'">
        <section class="settings-section">
          <div class="section-header">
            ...
            <div class="section-desc">
              How the smart router selects the best model for each request. The matrix score is read
              at inference time against the active profile and use case.
            </div>
          </div>
          <div class="routing-strategies">
            <label
              v-for="s in ROUTING_STRATEGIES"
              :key="s.value"
              class="strategy-row"
          ...
          <div v-if="store.config.routing_strategy === 'progression'" class="alpha-control">
            <label class="field-label"
              >α — balance instantaneous score vs trajectory
              <span class="field-hint">(0 = trajectory only · 1 = score only)</span></label
            >
            <input
              type="range"
              v-model.number="store.config.alpha"
              min="0"
              max="1"
              step="0.05"
              class="alpha-slider"
          ...
          <div class="thresholds-grid">
            ...
            <div class="threshold-field">
              <label class="field-label"
                ...
                <span class="field-hint">— σ ≥ ε flags for human review</span></label
              >
              <input
                type="number"
                v-model.number="store.config.variance_threshold"
                class="field-input"
                placeholder="e.g. 0.1"
                min="0"
                max="1"
...
.criterion-block:hover {
  background: var(--bg-3);
}
.criterion-block.disabled {
  opacity: 0.45;
}
.criterion-row {
  display: flex;
...
.panel-criterion-chip input {
  display: none;
}
.panel-criterion-chip.active {
  color: var(--accent);
...
.arena-judge-row:hover {
  background: var(--bg-3);
}
.arena-judge-row input {

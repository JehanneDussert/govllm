...
function extractText(raw: string): string {
  ...
  try {
    ...
    const userMatch = raw.match(/'role':\s*'user',\s*'content':\s*'([^']+)'/i)
      || raw.match(/"role":\s*"user",\s*"content":\s*"([^"]+)"/i)
    if (userMatch) return userMatch[1]

    const contentMatch = raw.match(/'content':\s*'([^']{3,120})'/i)
      || raw.match(/"content":\s*"([^"]{3,120})"/i)
...
function extractOutput(raw: string): string {
  if (!raw) return '—'
  try {
    const contentMatch = raw.match(/'content':\s*["']([^"']{3,200})/i)
      || raw.match(/"content":\s*["']([^"']{3,200})/i)
...
.table-head .col { font-size: 10px; letter-spacing: 1px; color: var(--text-dim); }
...
.table-row.expanded { background: var(--bg-3); }
...
.tag-cyan   { color: var(--accent); border-color: rgba(0,229,255,.3); background: rgba(0,229,255,.06); }
.tag-purple { color: #a78bfa; border-color: rgba(167,139,250,.3); background: rgba(167,139,250,.06); }
.tag-green  { color: var(--green); border-color: rgba(63,185,80,.3); background: rgba(63,185,80,.06); }
.tag-orange { color: #f0883e; border-color: rgba(240,136,62,.3); background: rgba(240,136,62,.06); }

.latency-val, .score-val { font-family: var(--font-mono); font-size: 12px; }
.ok   { color: var(--green); }
...
.expanded-row { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }
.expanded-block { display: flex; flex-direction: column; gap: 4px; }
...
.expanded-meta { font-size: 10px; color: var(--text-dim); display: flex; gap: 8px; }

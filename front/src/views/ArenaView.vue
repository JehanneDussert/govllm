...
function labelFor(id: string) {
  return (
    criteriaLabels.value[id] ?? judgeStore.config?.criteria.find((c) => c.id === id)?.label ?? id
  )
}
...
async function runArena() {
  ...
  const judgeModels =
    inputMode.value === 'auto' && generatorModel.value
...
function startValidityPoll() {
  ...
  validityPollInterval = setInterval(async () => {
    if (activeTab.value !== 'corpus' || !showValidity.value) {
      stopValidityPoll()
      return
    }
    try {
...
function stopValidityPoll() {
  if (validityPollInterval) {
    clearInterval(validityPollInterval)
    validityPollInterval = null
  }
...
async function loadIncoherenceItems() {
  ...
  try {
    ...
    incoherenceLoading.value = false
  }
}
...
watch([validityFilterJudge, validityFilterCriterion], () => {
...
const validityPivot = computed(() => {
  ...
  const criteria = validityFilterCriterion.value
    ? [validityFilterCriterion.value]
  ...
  for (const e of entries) {
    ...
    if (!map[e.judge_model][e.criterion])
      map[e.judge_model][e.criterion] = { rates: [], samples: 0 }
  ...
  const rows = judges.map((judge) => {
    ...
    return {
      judge,
      family:
        validityData.value!.entries.find((e) => e.judge_model === judge)?.judge_family ?? 'unknown',
      cells: criteria.map((crit) => {
        const cell = map[judge]?.[crit]
        if (!cell?.rates.length) return null as PivotCell | null
        return {
          mean_rate: cell.rates.reduce((a, b) => a + b, 0) / cell.rates.length,
...
const corpusSummary = computed(() => {
  ...
  return {
    ...
    flipPct: orderSensData.value?.length
      ? (orderSensData.value.reduce((acc, e) => acc + e.flip_rate, 0) /
          orderSensData.value.length) *
        100
      : null,
  }
})
...
const bestJudgePerCriterion = computed(() => {
  ...
  for (const c of validityPivot.value.criteria) {
    ...
    for (const row of validityPivot.value.rows) {
      ...
      if (cell && cell.mean_rate > bestRate) {
        bestRate = cell.mean_rate
        bestJudge = row.judge
      }
    }
...
function openValidityDrawer(judge: string, criterion: string) {
  if (validityDrawer.value?.judge === judge && validityDrawer.value?.criterion === criterion)
    validityDrawer.value = null
  else validityDrawer.value = { judge, criterion }
}
  ...
  return {
    ...
    tooltip: {
      ...
      formatter: (params: { value: [number, number] }[]) => {
        ...
        return `${date}<br/>σ = ${p.value[1].toFixed(4)}`
      },
    },
    xAxis: {
      type: 'time',
  ...
  return {
    ...
    tooltip: {
      ...
      formatter: (p: { data: [number, number, number, number, boolean] }) => {
        const d = p.data
        return (
          `<b>${judge_families[d[1]]}</b> → ${shortModels[d[0]]}<br/>` +
...
onMounted(async () => {
  ...
  api
    ...
    .then((res) => {
      ...
      if (res.data.models.length && !generatorModel.value)
        generatorModel.value = res.data.models[0] ?? ''
    })
    .catch(() => {})
})
onUnmounted(() => {
  ...
  <div class="arena-view">
    <div class="page-header">
      <div>
        ...
        <p class="page-sub">
          Each judge specialises on a regulatory domain — compare how they score the same response
        </p>
      </div>
    ...
    <div class="arena-tabs">
      <button class="arena-tab" :class="{ active: activeTab === 'run' }" @click="switchTab('run')">
        Run
      </button>
      <button
        class="arena-tab"
        :class="{ active: activeTab === 'variance' }"
        @click="switchTab('variance')"
      >
        Variance explorer
      </button>
      <button
        class="arena-tab"
        :class="{ active: activeTab === 'bias' }"
        @click="switchTab('bias')"
      >
        Bias matrix
      </button>
      <button
      ...
      <div class="prompt-box">
        ...
        <template v-if="inputMode === 'manual'">
          ...
          <textarea
            v-model="answer"
            class="prompt-input"
            placeholder="Answer to evaluate across all judges…"
            rows="3"
            :disabled="loading"
          />
        </template>
        ...
        <template v-else>
          <textarea
            v-model="question"
            class="prompt-input"
            placeholder="Enter a question — a model will generate the answer…"
            rows="2"
          ...
          <div v-if="generatorModel" class="auto-hint">
            <span class="auto-hint-model">{{ modelShortName(generatorModel) }}</span> generates the
            answer and is automatically excluded from the judge panel for this run.
          </div>
          <textarea
            v-model="answer"
            class="prompt-input"
        ...
        <div class="cards-grid">
          ...
          >
            ...
            <div class="card-header">
              <div class="card-identity">
                <div
                  class="avatar"
                  :style="{
                    background: judgeColor(shell.model_family) + '22',
                    color: judgeColor(shell.model_family),
                  }"
                >
            ...
            <div class="ring-wrapper">
              <svg width="72" height="72" viewBox="0 0 72 72">
                ...
                <text
                  ...
                  font-size="14"
                  font-weight="500"
                  :fill="judgeColor(shell.model_family)"
                >
            ...
            <div class="criteria-dots">
              ...
              <template v-if="!shell.done">
                <div v-for="cid in shell.assigned_criteria.slice(0, 4)" :key="cid" class="dot-row">
                  ...
                  <div class="dots">
                    <div
                      ...
                      :style="{
                        background: judgeColor(shell.model_family) + '44',
                ...
                >
                  ...
                  <div class="dots">
                    <div
                      ...
                      :style="{
                        background:
                          n <= Math.round(score.score * 5)
                            ? judgeColor(shell.model_family)
                            : judgeColor(shell.model_family) + '33',
                      }"
                    />
                  </div>
            ...
            >
              ⚡
              {{ ((incoherenceRates[shell.model_name]?.incoherence_rate ?? 0) * 100).toFixed(0) }}%
              incoh.
            </div>
            ...
            <div class="vote-btn-container">
              <button
                ...
                :style="
                  shell.done && isWinner(shell)
                    ? {
                        borderColor: judgeColor(shell.model_family) + '66',
                        color: judgeColor(shell.model_family),
                      }
                    : {}
                "
        ...
        <div v-if="sessionDone" class="sigma-section">
          ...
          <div class="sigma-explanation" v-if="sigma !== null">
            ...
            <template v-else-if="sigma <= 0.08">
              The judges show moderate disagreement — this is a regulatory grey zone worth human
              review.
            </template>
            <template v-else>
              Strong inter-judge disagreement detected — this response raises unresolved regulatory
              questions requiring human arbitration.
            </template>
          </div>
        </div>
      </template>
      ...
      <div class="chart-section">
        <div class="chart-header">
          ...
          <div class="chart-desc">
            Mean per-criterion variance across all judges per session. Low σ = consensus. High σ =
            regulatory grey zone requiring human review.
          </div>
        </div>
        <div v-if="varianceLoading" class="chart-loading">
          <div class="loading-dots"><span /><span /><span /></div>
        </div>
        <div v-else-if="!varianceData || !varianceData.points.length" class="chart-empty">
          No sessions recorded yet — run the Arena to generate data.
        </div>
    ...
    <template v-if="activeTab === 'bias'">
      <div class="chart-section">
        <div class="chart-header">
          ...
          <div class="chart-desc">
            Mean score given by each judge family to each evaluated model. Diagonal cells (same
            family) indicate self-preference (SPR). Significant off-diagonal asymmetry = epistemic
            discrimination.
          </div>
        </div>
        <div v-if="biasLoading" class="chart-loading">
          <div class="loading-dots"><span /><span /><span /></div>
        </div>
        <div v-else-if="!biasData || !biasData.cells.length" class="chart-empty">
          No cross-judge data yet — run the Arena with multiple judge families to generate the bias
          matrix.
        </div>
    ...
    <template v-if="activeTab === 'corpus'">
      <div class="corpus-section">
        <div class="chart-header">
          ...
          <div class="chart-desc">
            Run configured judges against expert-annotated cases in checklist mode. Each
            sub-question has a binary expected answer (True = compliant). Agreement rate measures
            judge validity, not just reliability.
          </div>
        </div>

        <!-- Loading -->
        <div v-if="corpusLoading" class="chart-loading">
          <div class="loading-dots"><span /><span /><span /></div>
        </div>
        ...
        <template v-else>
          <div class="corpus-controls">
            ...
            <div class="corpus-filter">
              ...
              <select
                class="selector corpus-case-select"
            ...
            <button
              class="run-btn corpus-run-btn"
              @click="runCorpus"
              :disabled="!selectedCaseId || corpusRunning"
            >
            ...
            <table class="corpus-table">
              <thead>
                <tr>
                  ...
                  <th v-for="(_, qid) in corpusResult.expected_answers" :key="qid" class="col-q">
                    {{ qid }}
                  </th>
              ...
              <tbody>
                ...
                <tr class="expected-row">
                  ...
                  <td v-for="(val, qid) in corpusResult.expected_answers" :key="qid" class="col-q">
                    <span class="answer-chip" :class="val ? 'ok' : 'fail'">{{
                      val ? '✓' : '✗'
                    }}</span>
                  </td>
                ...
                <tr v-for="j in corpusResult.judges" :key="j.judge_model">
                  <td class="col-judge judge-label">
                    <span
                      class="judge-family-dot"
                      :style="{ background: judgeColor(j.judge_family) }"
                    />
                    {{ modelShortName(j.judge_model) }}
                    <span v-if="j.reason" class="judge-reason" :title="j.reason">ⓘ</span>
                  </td>
                  <td
                    v-for="(expected, qid) in corpusResult.expected_answers"
                    :key="qid"
                    class="col-q"
                  >
                    <span
                      class="answer-chip"
                      :class="[
                        j.answers[qid] ? 'ok' : 'fail',
                        j.answers[qid] === expected ? 'match' : 'mismatch',
                      ]"
                      >{{ j.answers[qid] ? '✓' : '✗' }}</span
                    >
                  </td>
          ...
          <div class="validity-section">
            <button
              class="validity-toggle"
              @click="showValidity ? (showValidity = false) : loadValidity()"
              ...
              <div v-else class="validity-report">
                ...
                <div class="validity-header">
                  <div class="validity-summary">
                    <span
                      >{{ corpusSummary.nCases }} cases · {{ corpusSummary.nJudges }} judges ·
                      {{ corpusSummary.nEvals }} evaluations</span
                    >
                    <span
                      v-if="corpusSummary.flipPct !== null"
                      class="summary-chip"
                      :class="corpusSummary.flipPct >= 40 ? 'chip-warn' : 'chip-ok'"
                    >
                      ⇄ {{ corpusSummary.flipPct.toFixed(0) }}% flip
                    </span>
                    <span
                      v-if="corpusSummary.incoherenceTotal > 0"
                      class="summary-chip chip-neutral"
                    >
                      ⚠ {{ corpusSummary.incoherenceTotal }} incoh.B
                      <template
                        v-if="
                          corpusSummary.incoherenceConfirmed +
                            corpusSummary.incoherenceInvalidated >
                          0
                        "
                      >
                        ({{ corpusSummary.incoherenceConfirmed }}✓
                  ...
                  <div class="validity-filters">
                    <div class="corpus-filter">
                      ...
                      <select class="selector" v-model="validityFilterJudge">
                        <option value="">All</option>
                        <option v-for="j in validityJudges" :key="j" :value="j">
                          {{ modelShortName(j) }}
                        </option>
                      </select>
                    </div>
                ...
                <div v-if="runProgress" class="gt-progress">
                  <div class="gt-progress-header">
                    ...
                    <span class="gt-progress-pct"
                      >{{ runProgress.done }} / {{ runProgress.total }} evaluations ·
                      {{ runProgress.pct }}%</span
                    >
                    <span
                      v-if="validityPolling"
                      class="gt-refresh-dot"
                      title="Auto-refresh actif (30s)"
                    />
                    <button
                      class="reset-progress-btn"
                      @click="resetProgressSnapshot"
                      title="Reset baseline to now (use when starting a new run)"
                    >
                      ↺
                    </button>
                  </div>
                  <div class="gt-progress-track">
                    <div
                      ...
                      :class="runProgress.pct >= 100 ? 'complete' : ''"
                    />
                  </div>
                </div>
                ...
                <div class="corpus-results-title">
                  SCORE GLOBAL — mean agreement per judge × criterion (click cell for detail)
                </div>
                <div class="pivot-scroll">
                  <table class="corpus-table">
                    <thead>
                      <tr>
                        <th class="col-judge">Judge</th>
                        <th v-for="c in validityPivot.criteria" :key="c" class="col-crit">
                          {{ c }}
                        </th>
                    ...
                    <tbody>
                      <tr v-for="row in validityPivot.rows" :key="row.judge">
                        <td class="col-judge judge-label">
                          <span
                            class="judge-family-dot"
                            :style="{ background: judgeColor(row.family) }"
                          />
                          {{ modelShortName(row.judge) }}
                        </td>
                        <td
                          v-for="(cell, ci) in row.cells"
                          :key="validityPivot.criteria[ci]"
                          class="col-crit pivot-cell"
                          :class="[
                            agreeClass(cell?.mean_rate ?? null),
                            validityDrawer?.judge === row.judge &&
                            validityDrawer?.criterion === validityPivot.criteria[ci]
                              ? 'pivot-active'
                              : '',
                            cell === null ? 'pivot-empty' : '',
                          ]"
                          :title="
                            cell ? `n=${cell.total_samples} — click to drill down` : 'No data'
                          "
                          @click="
                            cell !== null &&
                            openValidityDrawer(row.judge, validityPivot.criteria[ci])
                          "
                        >
                          <span v-if="cell !== null">
                            {{ (cell.mean_rate * 100).toFixed(0) }}%
                            <span
                              v-if="bestJudgePerCriterion[validityPivot.criteria[ci]] === row.judge"
                              class="best-crown"
                              title="Best judge for this criterion"
                              >★</span
                            >
                          </span>
                        ...
                        <td class="col-score" :class="agreeClass(row.global_mean)">
                          {{
                            row.global_mean !== null
                              ? (row.global_mean * 100).toFixed(0) + '%'
                              : '—'
                          }}
                          <span
                            v-if="bestJudgeGlobal === row.judge && row.global_mean !== null"
                            class="best-crown"
                            title="Best overall judge"
                            >★</span
                          >
                        </td>
                      </tr>
                ...
                <div v-if="validityDrawer" class="validity-drawer">
                  ...
                  <div class="drawer-questions">
                    <div
                      v-for="e in validityDrawerEntries"
                      :key="e.question_id"
                      class="drawer-q-row"
                    >
                      ...
                      <div class="drawer-bar-track">
                        <div
                          ...
                          :style="{
                            width: `${e.agreement_rate * 100}%`,
                            background:
                              e.agreement_rate >= 0.75
                                ? '#1d9e75'
                                : e.agreement_rate >= 0.5
                                  ? '#ba7517'
                                  : '#e24b4a',
                          }"
                        />
                      </div>
                      <span class="drawer-rate" :class="agreeClass(e.agreement_rate)"
                        >{{ (e.agreement_rate * 100).toFixed(0) }}%</span
                      >
                ...
                <div class="order-sens-section">
                  <div class="order-sens-header">
                    <span class="corpus-results-title"
                      >ORDER SENSITIVITY — flip rate original vs reversed</span
                    >
                    <button
                      v-if="!orderSensLoading"
                      class="validity-toggle"
                      @click="
                        orderSensData === null
                          ? loadOrderSensitivity()
                          : (showOrderSens = !showOrderSens)
                      "
                    >
                      {{ orderSensData === null ? '↓ Load' : showOrderSens ? '↑ Hide' : '↓ Show' }}
                    </button>
                    <span v-if="orderSensLoading" class="loading-dots"
                      ><span /><span /><span
                    /></span>
                  </div>
                  <template v-if="orderSensData !== null && showOrderSens">
                    <div v-if="!orderSensData.length" class="chart-empty" style="padding: 20px 0">
                      No reversed runs found — run checklist with
                      <code>question_order=reversed</code> first.
                    </div>
                      ...
                      <tbody>
                        <tr v-for="e in filteredOrderSens" :key="`${e.judge_model}-${e.criterion}`">
                          ...
                          <td :class="e.mean_delta_agreement >= 0 ? 'agree-good' : 'agree-bad'">
                            {{ e.mean_delta_agreement >= 0 ? '+' : ''
                            }}{{ (e.mean_delta_agreement * 100).toFixed(0) }}%
                          </td>
                ...
                <div class="order-sens-section">
                  <div class="order-sens-header">
                    <span class="corpus-results-title"
                      >INCOHERENCE VALIDATION — pattern B (compliant answer + negative reason)</span
                    >
                    <button
                      v-if="!incoherenceLoading"
                      class="validity-toggle"
                      @click="
                        incoherenceItems === null
                          ? loadIncoherenceItems()
                          : (showIncoherence = !showIncoherence)
                      "
                    >
                      {{
                    ...
                    <span v-if="incoherenceLoading" class="loading-dots"
                      ><span /><span /><span
                    /></span>
                  </div>
                  <template v-if="incoherenceItems !== null && showIncoherence">
                    <div
                      v-if="!filteredIncoherence.length"
                      class="chart-empty"
                    ...
                    <div v-else class="incoh-table-wrap">
                      <table class="corpus-table incoh-table">
                        ...
                        <tbody>
                          <tr
                            v-for="item in filteredIncoherence"
                            :key="item.result_id"
                            :class="
                              item.incoherence_validated === true
                                ? 'incoh-confirmed'
                                : item.incoherence_validated === false
                                  ? 'incoh-invalidated'
                                  : ''
                            "
                          >
                            <td class="col-judge judge-label">
                              <span
                                class="judge-family-dot"
                                :style="{ background: judgeColor(item.judge_family) }"
                              />
                              {{ modelShortName(item.judge_model) }}
                            </td>
                            <td class="incoh-col-case" :title="item.prompt_preview">
                              {{ item.prompt_preview.slice(0, 45) }}…
                            </td>
                            <td>{{ item.criterion }}</td>
                            <td class="incoh-col-answers">
                              <span
                                v-for="(val, qid) in item.answers"
                                :key="qid"
                                class="incoh-answer-pill"
                                :class="
                                  val === item.expected_answers[qid] ? 'pill-match' : 'pill-miss'
                                "
                              >
                                {{ qid }}:{{ val ? 'T' : 'F' }}
                              </span>
                            </td>
                            <td class="incoh-col-reason" :title="item.reason ?? ''">
                              {{ (item.reason ?? '').slice(0, 80)
                              }}{{ (item.reason?.length ?? 0) > 80 ? '…' : '' }}
                            </td>
                            <td class="incoh-col-status">
                              <span
                                v-if="item.incoherence_validated === true"
                                class="incoh-badge confirmed"
                                >confirmed</span
                              >
                              <span
                                v-else-if="item.incoherence_validated === false"
                                class="incoh-badge invalidated"
                                >false pos.</span
                              >
                            ...
                            <td class="incoh-col-actions">
                              <button
                                class="incoh-btn confirm"
                                :class="{ active: item.incoherence_validated === true }"
                                @click="
                                  setIncoherenceValidation(
                                    item.result_id,
                                    item.incoherence_validated === true ? null : true,
                                  )
                                "
                                title="Confirm incoherence"
                              >
                                ✓
                              </button>
                              <button
                                class="incoh-btn invalidate"
                                :class="{ active: item.incoherence_validated === false }"
                                @click="
                                  setIncoherenceValidation(
                                    item.result_id,
                                    item.incoherence_validated === false ? null : false,
                                  )
                                "
...
.loading-dots span {
  ...
  border-radius: 50%;
  background: var(--bg);
  animation: arena-bounce 1.2s ease infinite;
}
...
.validity-toggle:hover:not(:disabled) {
  border-color: var(--accent);
  color: var(--accent);
}
.validity-report {
...
.pivot-cell:hover:not(.pivot-empty) {
...
.incoh-badge.confirmed {
  color: #e24b4a;
  border: 1px solid rgba(226, 75, 74, 0.3);
}
.incoh-badge.invalidated {
  color: #1d9e75;
...
.incoh-btn.confirm.active {
  background: rgba(226, 75, 74, 0.1);
  border-color: #e24b4a;
  color: #e24b4a;
}
.incoh-btn.invalidate.active {
  background: rgba(29, 158, 117, 0.1);
  border-color: #1d9e75;
  color: #1d9e75;
}

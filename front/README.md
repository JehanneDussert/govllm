...
| **Corpus** | Ground truth validity. Table: judge × criterion agreement (%). Click a cell for per-sub-question breakdown. Order sensitivity section (flip rate + Δ agreement when question order is reversed). Source: ground truth corpus of 34 annotated (prompt, response) pairs. |
